...
*/			
			$query_sel_bill = "select aa.*,aan.id,aan.is_new from tbl_affiliate_articles aa left join (select * from tbl_affiliate_articles_notifications where member_id=$member_id) aan using(article_id) where aa.affiliate_id in (". $list . ") and aa.created=CURDATE() order by aa.article_id desc";
