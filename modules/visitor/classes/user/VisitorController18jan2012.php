<?php
   session_start();
	/***********************************************************************************************************
		...
		* FileName			: VisitorController.php	
	...
	class VisitorController extends UserController
	{
		...
		*/
		function SaveOneTimeSubscriber()
		{
			...
			*/		    
			...
			$oVisitor->first_name		=	$_POST['first_name'];
			$oVisitor->last_name		=	$_POST['last_name'];
			$oVisitor->email			=	trim($_POST['email']);
			$oVisitor->promo_code		=	$_POST['promo_code'];
			
			$oVisitor->mail_address		=	htmlentities($_POST['address'],ENT_QUOTES);
			$oVisitor->mail_state	    =	htmlentities($_POST['state'],ENT_QUOTES);
			$oVisitor->mail_city        =	htmlentities($_POST['city'],ENT_QUOTES);
			$oVisitor->mail_postal_code	=	htmlentities($_POST['zip_code'],ENT_QUOTES);
		

			$oVisitor->onetimefee	=	$_POST['onetimefee'];
			$oVisitor->article_id	=	$_POST['article_id'];
			...
			$oMember = $this->oModel->getMemeberType($oVisitor->promo_code);
			$oMember = $oMember[0];
			$oVisitor->member_type = $oMember->name;
			
			$id = $this->oModel->addOneTimeSubscriber($oVisitor);
			if($id) {
			
				$oVisitor->visitor_id	=	$id;
				$this->oneTimePaymentStepTwo($oVisitor);
				die();				
			} else {
		...
		*******************************************************************************************/
		function oneTimePaymentStepTwo($oVisitor = "")
		{
			...
			$oPaypal->return 		= $_CONF['SiteUrl']."/index.php?stage=visitor&mode=petitionPaymentSuccess&article_id=".$oVisitor->article_id."&visitor_id=" . base64_encode($oVisitor->visitor_id);			
			$oPaypal->cancel_return =   $_CONF['SiteUrl']."/index.php?stage=visitor&mode=petitionPaymentCancel&article_id=".$oVisitor->article_id."&visitor_id=" . base64_encode($oVisitor->visitor_id);
			$oPaypal->notify_url	=   $_CONF['SiteUrl']."/index.php?stage=visitor&mode=petitionpaymentipn&article_id=".$oVisitor->article_id."&visitor_id=" . base64_encode($oVisitor->visitor_id);
		...
		*/
		function petitionPaymentSuccess()
		{
			...
			//echo $sSQL;
			$oMember = $this->oModel->getVisitorAddressDetails($visitor_id);
			$oMember = $oMember[0];
			...
			if(is_object($oMember)) {
				...
				if($isValidCicero) {
					...
					##Update database 			
					$rs = $this->oModel->setUserTypeOneTimeSubscriber($visitor_id);
					echo "<script>location='index.php?stage=visitor&mode=EmailMessage&message=invalidaddressonetime'</script>";
					die();						
				}//if
			} else {
		...
		*/
		...
		{								
			...
			echo "<script>location='index.php?stage=visitor&mode=EmailMessage&message=petitionpaymentcancel'</script>";
			die();				
		}//ef
		...
		*/
		function AffiliatePetitionsDetailSucess($article_id, $visitor_id) {
			...
			$this->oView->affiliateArticle = $this->affModel->getArticleDetailOneTime($article_id);			 
		...
		*/
		function signPetitionOneTime() 
		{
			...
			if( isset($_REQUEST['articleID']) && isset($_REQUEST['visitor_id']))
			{
				...
				//echo $sSQL;
				$oVisitor = $this->oModel->getVisitorAddressDetails($visitor_id);
				$oVisitor = $oVisitor[0];
