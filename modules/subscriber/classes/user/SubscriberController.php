		...
		{	
			...
			foreach($affiliateArticles as $key=>$value) {
				...
				if($affiliateArticles[$key]["article_type"] == "bill") {					
					...
					elseif($affiliateArticles[$key]["article_type"] == "news")
					{
						array_push($final_alert_array, $affiliateArticles[$key]);
						$ctr = $ctr + 1;
	...
	/******************************************************************************************
		...
		*/
		function AffiliateDetails($affID)
		{
			...
			if($_SESSION['member_type']=='subscriber' || $_SESSION['member_type']=='observer')	
			{		
			...
			{
				$affiliatesID = $this->affModel->getMyAffiliatesID($uID);
				$reqAffiliatesID = $this->affModel->getReqAffiliatesID($uID);	
				if(array_key_exists($affID, $affiliatesID))	{
				$this->oView->view_status=$affiliatesID[$affID];
				$this->oView->add_affiliate = true; 	
				}
				else if(array_key_exists($affID, $reqAffiliatesID))	{
				$this->oView->view_req_status=$reqAffiliatesID[$affID];
				$this->oView->add_affiliate = true; 	
				} else {
				$this->oView->add_affiliate = false; 	
				}
		...
		*/
		...
		function PetitionsDetail($articleID = NULL) {
			...
			$this->oView->site_url=$_CONF['SiteUrl'];						
			...
			if($_SESSION['member_type']=='affiliate')
			{
				...
				$associate_article_id=$this->affModel->getAssociatedArticleId($affID,$aff_id,$articleID);
				if($associate_article_id)
					$this->oView->ArticleAssociation = true;
				else
					$this->oView->ArticleAssociation = false;
			}	
			// if voting ended
		...
		*/
		function AffiliateVoteAlertsDetail($articleID = NULL)	{
			...
			$articleDetails=$this->affModel->getAffiliateArticleDetail($affID ,$articleID);
			$this->oView->affiliateArticle = $articleDetails;
			if($_SESSION['member_type']=='affiliate')
			{
				$aff_id=$_SESSION['member_id'];
				$associate_article_id=$this->affModel->getAssociatedArticleId($affID,$aff_id,$articleID);
		...
		*/
		function AffiliatePetitionsDetailAffiliate($articleID = NULL)	{
			...
			if($_SESSION['member_type']=='affiliate')
			{
				$aff_id=$_SESSION['member_id'];
				$associate_article_id=$this->affModel->getAssociatedArticleId($affID,$aff_id,$articleID);
				if($associate_article_id==true || !empty($associate_article_id))
					$this->oView->ArticleAssociation = true;
				else
					$this->oView->ArticleAssociation = false;
			}		
