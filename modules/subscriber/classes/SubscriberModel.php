	...
	***************************************************************/
	class SubscriberModel extends Model
	{
		...
		****************************************************************************************/		
		...
		{
			$query = "select m.first_name, m.last_name, m.email,m.reg_date, s.mail_street_address, s.mail_city,
	...
	/*
		...
		*/
		...
		{			
			...
			if(count($list))
			{
				...
				{			
					$start = ($page - 1)*$limit;			
					$query = "select aa.*,aan.id,aan.is_new from tbl_affiliate_articles aa left join (select * from tbl_affiliate_articles_notifications where member_id=$uID) aan using(article_id) where aa.affiliate_id in (". $list . ") order by aa.article_id desc limit  $start , $limit";
				}
				else
				{			
					## $page =0 i.e. show all elements 
					$query = "select aa.*,aan.id,aan.is_new from tbl_affiliate_articles aa left join (select * from tbl_affiliate_articles_notifications where member_id=$uID) aan using(article_id) where aa.affiliate_id in (". $list . ") order by aa.article_id desc";
				}
		...
		**************************************************************************************
		*Function Header
		*Function Name  : AddSubscriberAdmin
		*Function Type  : Call by value	
		...
		***************************************************************************************
		*/		
		function AddSubscriberAdmin($oSubscriber)
		{
...
function getStates()
{
	$query="select name from tbl_states";
	$rs = $this->Execute($query);
	$states=array();
	while($row=$rs->FetchRow())
	{
