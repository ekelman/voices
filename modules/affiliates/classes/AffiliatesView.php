<?php
	...
	***************************************************************/
	...
	class AffiliatesView extends AdminView
	{
		...
		*/
		...
		{
			global $_CONF;
			$this->obTemplate->clear_cache('affiliates/ihtml/add_affiliate.html');		 
			
			$this->obTemplate->assign('sErrorMsg', $this->sErrorMsg);	
			$this->obTemplate->assign('oAffiliate', $this->oAffiliate);				
			
			$this->obTemplate->assign('Content',$this->obTemplate->fetch('affiliates/ihtml/add_affiliate.html'));
			$this->parse();
		}//ef
		/*
		**************************************************************************************
...
";
		...
		*/
		function ListAffiliates()
		{
			...
			$OHeadCell[++$i]					= new gCell();
			$OHeadCell[$i]->iType				= GRID_ELE_TEXT;
			$OHeadCell[$i]->vProperty['Text']	= 'Organization Name';
			$OHeadCell[$i]->vProperty['Sort']	= false;
			...
			$OHeadCell[++$i]					= new gCell();
			$OHeadCell[$i]->iType				= GRID_ELE_TEXT;
			$OHeadCell[$i]->vProperty['Text']	= 'Payment';
			$OHeadCell[$i]->vProperty['Sort']	= false;
			
			$OHeadCell[++$i]					= new gCell();
			$OHeadCell[$i]->iType				= GRID_ELE_TEXT;
			$OHeadCell[$i]->vProperty['Text']	= 'Details';
			$OHeadCell[$i]->vProperty['Sort']	= false;
			...
			$ODataCell[++$i] 					= new gCell();
			$ODataCell[$i]->iType 				= GRID_ELE_IMAGE_LINK;
			$ODataCell[$i]->vProperty['Text']	= 'View';
			
			$ODataCell[$i]->vProperty['Image']	= $_CONF[ImageUrl].'icons/details.gif';
			$ODataCell[$i]->vProperty['Link']	= 'index.php';
			$ODataCell[$i]->vProperty['FixedQueryString']		= 	array('stage'=>'affiliates','mode'=>'ListAffiliatePayment');
			$ODataCell[$i]->vProperty['FloatingQueryString']	= 	array('id'=>'member_id');
			$ODataCell[$i]->vProperty['bNewWindow']				= 	false;
			...
			$ODataCell[$i]->vProperty['Image']	= $_CONF[ImageUrl].'icons/details.gif';
			$ODataCell[$i]->vProperty['Link']	= 'index.php';
			$ODataCell[$i]->vProperty['FixedQueryString']		= 	array('stage'=>'affiliates','mode'=>'AdminAffiliateDetails');
			$ODataCell[$i]->vProperty['FloatingQueryString']	= 	array('id'=>'member_id');
			$ODataCell[$i]->vProperty['bNewWindow']				= 	false;
		
			
			//operations displayed in the dropdown box like activate,deactivate and delete account	
			$SVerticalOperation['Text'] 		= 	array('Activate', 'Deactivate', 'Delete', 'PromoCode');
			$SVerticalOperation['Operation'] 	= 	array('ActivateAffiliate','DeactivateAffiliate','DeleteAffiliate','PromoCodeAffiliate');
			$SVerticalOperation['Confirm'] 		= 	array(0, 0, 1, 0);
			
			
			$oGrid = new Grid();
			$oGrid->sTitle 			=	"List of all Affiliates";
			$oGrid->sDescription 	= 	"&nbsp;";
			$oGrid->sStage 			= 	"affiliates";
			$oGrid->oHeadRow 		= 	$OHeadCell;
			$oGrid->oDataRow		= 	$ODataCell;
			$oGrid->sIdentifier 	= 	'member_id';
			$oGrid->oTemplate		= 	&$this->obTemplate;
			...
			$this->obTemplate->assign('search_field', $this->search_field);
			$this->obTemplate->assign('search_text', $this->search_text);
			$this->obTemplate->assign('CustomForm', $this->obTemplate->fetch("affiliates/ihtml/search_fields.html"));
			$this->obTemplate->assign('Content', $oGrid->build());
			$this->parse();			
		}//ef
		...
		*Function Type  : Call by value	
		*Functionality  : get affiliate details
		*Input			: nothing
		*Output			: get affiliate details
		*Return Value   : nothing
		*Note			: this function uses grid class to generate a list of all the Employers.
		***************************************************************************************
		*/
		function AdminAffiliateDetails()
		{
			global $_CONF;			
			$this->obTemplate->clear_cache('affiliates/ihtml/admin_affiliate_details.html');
			$this->obTemplate->assign('oAffiliate',$this->oAffiliate);
			$this->obTemplate->assign('Content',$this->obTemplate->fetch('affiliates/ihtml/admin_affiliate_details.html'));
			$this->parse();
		} //ef
		...
		*/
		function ListAffiliatePayment()
		{
			...
			$OHeadCell[++$i]					= new gCell();
			$OHeadCell[$i]->iType				= GRID_ELE_TEXT;
			$OHeadCell[$i]->vProperty['Text']	= 'Full Name';
			$OHeadCell[$i]->vProperty['Sort']	= false;
			...
			$OHeadCell[++$i]					= new gCell();
			$OHeadCell[$i]->iType				= GRID_ELE_TEXT;
			$OHeadCell[$i]->vProperty['Text']	= 'Annual Subsciption';
			$OHeadCell[$i]->vProperty['Sort']	= false;
			...
			$ODataCell[$i]->vProperty['Col'] 	= 'amount';
			
			$ODataCell[++$i] 					= new gCell();
			$ODataCell[$i]->iType 				= GRID_ELE_COL;
			$ODataCell[$i]->vProperty['Col'] 	= 'date';


			//operations displayed in the dropdown box like activate,deactivate and delete account	
			/*
			$SVerticalOperation['Text'] 		= 	array('Activate', 'Deactivate', 'Delete');
			$SVerticalOperation['Operation'] 	= 	array('ActivateAffiliate','DeactivateAffiliate','DeleteAffiliate');
			$SVerticalOperation['Confirm'] 		= 	array(0, 0, 1);
			*/
			$this->amount  			=	'';	
			
			$oGrid = new Grid();
			$oGrid->sTitle 			=	"List of all Subscribers ";
			$oGrid->sDescription 	= 	""; // "Total Amount &nbsp; >> $" .$this->amount;
			$oGrid->sStage 			= 	"affiliates";
			$oGrid->oHeadRow 		= 	$OHeadCell;
			$oGrid->oDataRow		= 	$ODataCell;
			$oGrid->sIdentifier 	= 	'member_id';
