...
*/
		...
		**/
		...
		function AssociateAffiliate()
		{
				$this->AffiliateAuthenticate();
			
				if(isset($_REQUEST["id"]))
				$associate_aff_id = $_REQUEST["id"];
				$aff_id=$_SESSION['member_id'];
				$assoc_id = $this->oModel->addAffiliateAssociation($associate_aff_id,$aff_id);
				if($assoc_id>0)
				header("location:".$_CONF['SiteUrl']."/index.php?stage=subscriber&mode=AffiliateDetails&id=".$associate_aff_id);
							

		}//ef
		function AssociateReqAffiliate()
		{
				$this->AffiliateAuthenticate();
			
				if(isset($_REQUEST["id"]))
				$aff_id = $_REQUEST["id"];
				$associate_aff_id=$_SESSION['member_id'];
				$this->oModel->ConfirmAssociation($aff_id,$associate_aff_id);
				header("location:".$_CONF['SiteUrl']."/index.php?stage=subscriber&mode=AffiliateDetails&id=".$aff_id);
							

		}//ef
		function CancelReqAffiliate()
		{
				$this->AffiliateAuthenticate();
				...
				$aff_id = $_REQUEST["id"];
				$associate_aff_id=$_SESSION['member_id'];
				$assoc_id = $this->oModel->CancelAssociation($aff_id,$associate_aff_id);
		...
		function MyAffiliates()
		{
			...
			$profile_info = $this->oModel->getAffiliate($_SESSION['member_id']);
			$uID = $profile_info->member_id;			
			$this->oView->uProfile = $profile_info;	
			$arrAffiliates=$this->oModel->getMyAffiliates($uID);
			//print_r($this->oModel->getMyAffiliates($uID));
			//exit;
			foreach($arrAffiliates as $aff)
			{
				if($aff['status']=='confirmed')
				{
					$myAffiliates[]=$aff;
				}
				else
			...
			$this->oView->myAffiliates = $myAffiliates;
			$this->oView->reqAffiliates = $reqAffiliates;
		...
		function confirmAffAssociation()
		{
				$this->AffiliateAuthenticate();
			
				if(isset($_REQUEST["id"]))
				$aff_id = $_REQUEST["id"];
				$associated_aff_id=$_SESSION['member_id'];
				$this->oModel->ConfirmAssociation($aff_id,$associated_aff_id);
				header("location:".$_CONF['SiteUrl']."/index.php?stage=affiliates&mode=MyAffiliates");

		}//ef	
		function cancelAffAssociation()
		{
				$this->AffiliateAuthenticate();
				...
				$aff_id = $_REQUEST["id"];
				$associated_aff_id=$_SESSION['member_id'];
				$this->oModel->CancelAssociation($aff_id,$associated_aff_id);
				header("location:".$_CONF['SiteUrl']."/index.php?stage=affiliates&mode=MyAffiliates");

		}//ef		
		function removeAffAssociation()
		{
				$this->AffiliateAuthenticate();
			
				if(isset($_REQUEST["id"]))
				$aff_id1 = $_REQUEST["id"];
				$aff_id2=$_SESSION['member_id'];
				$this->oModel->removeAssociation($aff_id1,$aff_id2);
				header("location:".$_CONF['SiteUrl']."/index.php?stage=affiliates&mode=MyAffiliates");

		}//ef	
		function AddAffiliateArticle()
		{
			$this->AffiliateAuthenticate();
			if(isset($_REQUEST["articleID"]))
			{
				...
				$affID=$_SESSION['member_id'];
				$newID=$this->oModel->AddArticleId($affID,$articleID);
				if($article_type=='petition')
					$mode='AffiliatePetitionsDetailAffiliate';
				else if($article_type=='news')
					$mode='NewsDetail';
				else if($article_type=='bulletin')
					$mode='BulletinsDetail';
				else 
					$mode='AffiliateVoteAlertsDetail';
				
				$message = $article_type." added successfully";
				header("location:".$_CONF['SiteUrl']."/index.php?stage=subscriber&mode=".$mode."&affID=".$aff_id."&id=".$articleID."&msg=".$message);
				die();
		...
		**************************************************************************************
		*function header
		*function name  : AddAffiliatesMain
		*function type  : call by value	
		...
		***************************************************************************************
		*/
		function AddAffiliatesMain($oAffiliate,$sErrorMsg="")
		{
			$this->oView->sErrorMsg = $sErrorMsg;
			$this->oView->oAffiliate = $oAffiliate;			
			$this->oView->AddAffiliatesMain();
		}//ef
		
		/*
		**************************************************************************************
		*function header
		*function name  : SaveAffiliateMain
		*function type  : call by value	
		*functionality  : update the account of employer
		*input			: oEmployer array
		*output			: update the account of employer
		...
		*/
		...
		{  
			...
			if($isDuplicate == "email") {
				...
				$sErrorMsg	= "E-mail Id already exists.";
				$this->AddAffiliatesMainFinal($oAffiliate,$sErrorMsg);
				die();			
			...
			if($banner_name != "") {
				...
				if($isValidFileType && $isValidFileSize) {
					...
					if($result) {
						...
						$rs = $this->oModel->addAffiliateMain($oAffiliate);
						...
						if($rs) {
							...
							$sErrorMsg	= "Affilate has been added.";
         					$this->AddAffiliatesMainFinal($oAffiliate,$sErrorMsg);
							die();												
						} else {
				...
				if($rs) {	
					$sErrorMsg	= "Affilate has been added.";
					//header("location:".$_CONF['SiteUrl']."/index.php?stage=affiliates&mode=AddAffiliatesMainFinal");
					$this->AddAffiliatesMainFinal($oAffiliate,$sErrorMsg);
					die();												
				} else {
					$sErrorMsg	= "SQL error, Please try again.";
		...
		**************************************************************************************
		*function header
		*function name  : AddAffiliatesMainFinal
		*function type  : call by value	
		*functionality  : display the details of Affiliate
		*input			: member id
		*output			: display the details of Affiliate
		...
		***************************************************************************************
		*/
		function AddAffiliatesMainFinal($oAffiliate,$sErrorMsg)
		{
			$this->oView->sErrorMsg = $sErrorMsg;
			$this->oView->oAffiliate = $oAffiliate;			
			$this->oView->AddAffiliatesMainFinal();
		}//ef
