<?php
	...
	***************************************************************/
	class AffiliatesModel extends Model
	{
		...
		**************************************************************************************
		*Function Header
		*Function Name  : addAffiliateMain
		*Function Type  : Call by value	
		*Functionality  : add a employer from site	
		*Input			: object $oSubscriber containing subscriber's basic information
		*Output			: adds a record in database
	...
	/*
		...
		*/
		...
		function getSubscribers($aff_id)
		{
			$ss_query="select tm.first_name, tm.last_name, tm.email from tbl_member tm inner join (select subscriber_id from tbl_subscriber where prim_affiliate_id=$aff_id or find_in_set($aff_id,secondary_affiliates)) ts on (tm.member_id=ts.subscriber_id) where tm.isActive='y'";
			$ss_result = $this->Execute($ss_query);
			// return $this->_getPageArray($ss_result, 'Affiliate');
			$subscribers=array();
			while($row = $ss_result->FetchRow())
			{
				$subscribers[]=$row;
		...
		function getAffiliateBanner($aff_id)
		{
			$query="select tm.first_name, tm.last_name, ta.banner from tbl_member tm inner join tbl_affiliate ta on (tm.member_id=ta.affiliate_id) where affiliate_id=$aff_id";
			$rs = $this->Execute($query);
			$row = $rs->FetchRow();
			return $row;
		}
		
//New functions by Monika Bansal on 16-07-2012

	function getMyAffiliatesID($aff_id)
	{
		$query = "select associated_aff_id,status from tbl_affiliate_association where affiliate_id = $aff_id";
			$rs = $this->Execute($query);
			$arrAffiliates = array();
			if($rs->RecordCount()) {			
				while($row = $rs->FetchRow())
			...
			$arr=array_combine($arrAffiliates,$arrStatus);
			return  $arr;	
			
			

	}//ef
	function getReqAffiliatesID($aff_id)
	{
		$query = "select affiliate_id,status from tbl_affiliate_association where associated_aff_id = $aff_id";
			$rs = $this->Execute($query);
			$arrAffiliates = array();
			if($rs->RecordCount()) {			
				while($row = $rs->FetchRow())
	...
	function addAffiliateAssociation($associate_affID,$affID)
	{
		
		$query = "insert into tbl_affiliate_association(`affiliate_id`,`associated_aff_id`,`status`,`created`) values($affID,$associate_affID,'pending',now())";
		$rs = $this->Execute($query);
		$assoc_id=mysql_insert_id();
		return $assoc_id;
		...
		*/
		...
		function ConfirmAssociation($affID,$associatedaffID)
		{
			$query = "Update tbl_affiliate_association set status='confirmed' where affiliate_id=$affID and associated_aff_id=$associatedaffID";
			$rs = $this->Execute($query);
			return true;		
			
		}
		function CancelAssociation($affID,$associatedaffID)
		{
			$query = "delete from tbl_affiliate_association where affiliate_id=$affID and associated_aff_id=$associatedaffID";
			$rs = $this->Execute($query);
			return true;		
			
		}
		function removeAssociation($affID1,$affID2)
		{
			$query = "delete from tbl_affiliate_association where (affiliate_id=$affID1 || affiliate_id=$affID2) and (associated_aff_id=$affID1 || associated_aff_id=$affID2)";
			$rs = $this->Execute($query);
			$artquery="select article_id from tbl_affiliate_articles where affiliate_id=$affID2 and associated_article_id in (select article_id from tbl_affiliate_articles where affiliate_id=$affID1)";
			$artrs = $this->Execute($artquery);
			$artarr=array();
			while($row=$artrs->FetchRow())
			{
				$artarr[]=$row['article_id'];
			}
			if(count($artarr)>0)
			{
				$artarr=implode(',',$artarr);
				$artdel="delete from tbl_affiliate_articles where article_id in ($artarr) || associated_article_id in ($artarr)";
				$artrs1 = $this->Execute($artdel);
			}
				
			return true;		
			
		}
		function getAssociatedArticleId($affID1,$affID2,$article_id)
		{
			
			$rs_query="select * from tbl_affiliate_association where (affiliate_id=$affID1 || affiliate_id=$affID2) and (associated_aff_id=$affID1 || associated_aff_id=$affID2) and status='confirmed'";
			$rs1 = $this->Execute($rs_query);
			if($rs1->RecordCount())
			{
				$assocquery = "select associated_article_id from tbl_affiliate_articles where article_id=$article_id";
				$assocrs = $this->Execute($assocquery);
				$assocrow 	= $assocrs->FetchRow();
				
				if($assocrow['associated_article_id']!='' || $assocrow['associated_article_id']!=NULL)
				{
				
					$artquery="select article_id from tbl_affiliate_articles where affiliate_id=$affID2 and (article_id=".$assocrow['associated_article_id']." || associated_article_id=$article_id)";
					$artrs = $this->Execute($artquery);
					$row=$artrs->FetchRow();
				...
				{
					$query = "select article_id from tbl_affiliate_articles where affiliate_id=$affID2 and associated_article_id=$article_id";
					$rs = $this->Execute($query);
					if($rs->RecordCount())
					{
						$row=true;
		...
		function AddArticleId($aff_id,$article_id)
		{
			$query1="select * from tbl_affiliate_articles where article_id=$article_id";
			$rs1 = $this->Execute($query1);
			$row 	= $rs1->FetchRow();
			$query2="insert into tbl_affiliate_articles(`affiliate_id`,`article_title`,`bill_number`,`legtype`,`indication_position`,`voting_start`,`voting_end`,`allow_voting`,`support_file`,`affiliate_comment`,`created`,`indication_of_position`,`article_type`,`publish_from`,`publish_to`,`allow_comments`,`comment_period_from`,`comment_period_to`,`vote_alert_sent`,`petition_level`,`petition_state`,`associated_article_id`) values(".$aff_id.",'".$row['article_title']."','".$row['bill_number']."','".$row['legtype']."','".$row['indication_position']."','".$row['voting_start']."','".$row['voting_end']."','".$row['allow_voting']."','".$row['support_file']."','".$row['affiliate_comment']."',current_date(),'".$row['indication_of_position']."','".$row['article_type']."','".$row['publish_from']."','".$row['publish_to']."','".$row['allow_comments']."','".$row['comment_period_from']."','".$row['comment_period_to']."','".$row['vote_alert_sent']."','".$row['petition_level']."','".$row['petition_state']."',".$article_id.")";
			$rs2 = $this->Execute($query2);
			$newid= mysql_insert_id();
			$ss_query="select subscriber_id from tbl_subscriber where prim_affiliate_id='".$aff_id."' or find_in_set('".$aff_id."',secondary_affiliates)";
			$ss_result = $this->Execute($ss_query);
			while($row = $ss_result->FetchRow())
			{
				$new_query="insert into tbl_affiliate_articles_notifications(`article_id`,`member_id`, `is_new`) values ('".$newid."',".$row['subscriber_id'].",1)";
			$new_rs = $this->Execute($new_query);
			}
		...
		*************************************************************************************
		*Function Header
		*Function Name  : PromoCodeAffiliate
		*Function Type  : Call by value	
		*Functionality  : Activates a employer or list of employers		
		*Input			: Array of employer ids $IId
		*Output			: Activates a employer or list of employers
