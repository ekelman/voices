<?php
	...
	*******************************************************************/
	...
	class AffiliatesController extends AdminController{
		...
		***************************************************************************************
		*/
		function PromoCodeAffiliate()
		{
			global $_CONF;
			$_REQUEST['mode'] 		= "PromoCodeAffiliate";
			$promo_code	= $_REQUEST['p_code'];
			//$this->oModel->PromoCodeAffiliate($_REQUEST['identity'],$promo_code);
			$OEmailList = $this->oModel->PromoCodeAffiliate($_REQUEST['identity'],$promo_code);
			
			for($i = 0; $i < count($OEmailList); $i++)
			{
				$subject = "Promo code ".$promo_code." has been applied to your affiliate.";
				$body =	"Dear ".$OEmailList[$i]['first_name']." ".$OEmailList[$i]['last_name'].", promo code ".$promo_code." has been applied to your affiliate.";
				$email = $OEmailList[$i]['email'];
				SendMail(stripslashes(html_entity_decode($body)), $subject, array($email), $_CONF['adminEmail']);
			}
			
			$err="Promo Code has been updated Successfully.";
			//$this->ListAffiliates($OEmailList[0]["email"]);
			$this->ListAffiliates($err);
			die();			
		}
	}//ec
