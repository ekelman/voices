 ...
 *
 */
function smarty_function_pager1($params, &$smarty)
{
...
*/
	...
	foreach($minVars as $tmp)  {
		if (empty($params[$tmp]))	{
			$smarty->trigger_error('plugin "pager1": missing or empty parameter: "'.$tmp.'"');
		}
	}
	...
	if (!in_array($txt_pos, array('side', 'top', 'bottom'))) {
			$smarty->trigger_error('plugin "pager1": bad value for : "txt_pos"');
	}
	...
	$url = "http://".$_SERVER['HTTP_HOST'];	//	added by Krishan
	...
	if (($pos == 0)	AND ((!empty($txt_first)) OR !empty($img_first))){
		...
		foreach($pages as $num=>$url)	{
				...
				if($num == ($_REQUEST['pos']+1)) $txt= "<b>$num</b>";
				else $txt = $num;
				print '<a class="'.(($num == $short['now']) ? $class_numon : $class_num).'" href="'.$url.'">'.$txt.'</a>';
		}
		if ($txt_pos == 'side')	{
			print $cache['next'];;
		}
