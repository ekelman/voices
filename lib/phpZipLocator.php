...
class zipLocator
{
    ...
    */
    ...
    {
       global $db;
       $query = "SELECT * FROM tzipdata WHERE zipcode = '$zipOne'";
       ...
       $query = "SELECT * FROM tzipdata WHERE zipcode = '$zipTwo'";
    ...
    */
    ...
    {
        global $db;
        $query="SELECT * FROM tzipdata WHERE zipcode='$zip'";
        $db->query($query);
	       ...
	       if($db->affected_rows()<>0) {
			...
			//
			// $sql = "select zipcode, sqrt(power(69.1*(lat - $lat),2)+ power(69.1*(lon-$lon)*cos(lat/57.3),2)) as dist from  tzipdata " ; 
    		//$sql = $sql . " where sqrt(power(69.1*(lat - $lat),2)+ power(69.1*(lon-$lon)*cos(lat/57.3),2)) <= " . $radius ; 

	//
			//chnaged by sanjay
			$query="select zipcode from tzipdata as zip where (3958* 3.1415926*sqrt(($lat-lat)*($lat-lat) + cos($lat/57.29578)*cos(lat/57.29578)*($lon-lon)*($lon-lon))/180) <= $radius";

			
		
  			//die;
			// old 
            //$query="SELECT zipcode FROM tzipdata WHERE (POW((69.1*(lon-\"$lon\")*cos($lat/57.3)),\"2\")+POW((69.1*(lat-\"$lat\")),\"2\"))<($radius*$radius) ";
            $db->query($query);
            if($db->affected_rows()<>0) {
				$i	=	0;
